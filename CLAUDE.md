...
## Parity Status (as of 2026-03-07): ~87%
...
| Task Auto-Unblock                    | autoUnblockDependents() on task completion             | ~90%     | Active: blocked_by cleared when blocker completes |
...
- **Task Auto-Unblock (~90%)** — `autoUnblockDependents()` added to `handleUpdateTask`; actively clears `blocked_by` on task completion. 267/267 tests pass.
