...
export function handleRunPipeline(args) {
  ...
  const pipelineId = pipeline_id
    ? sanitizeId(pipeline_id, "pipeline_id")
    : `P${Date.now()}`;
  const pipelineDir = join(RESULTS_DIR, pipelineId);
  if (existsSync(pipelineDir))
    return text(
      `Pipeline ID ${pipelineId} already exists. Use a new pipeline_id.`,
    );
  ...
  normalizedTasks.forEach((task, i) => {
    writeFileSecure(
      join(pipelineDir, `${i}-${task.name}.prompt`),
      preamble + task.prompt + suffix,
    );
  });

  let script;
  const winSettingsFlag = existsSync(SETTINGS_FILE)
    ? `--settings ${batQuote(SETTINGS_FILE)}`
    : "";
  if (PLATFORM === "win32") {
  ...
  try {
    const runnerFile = join(
      pipelineDir,
      PLATFORM === "win32" ? "run.bat" : "run.sh",
    );
    openTerminalWithCommand(
      PLATFORM === "win32" ? `"${runnerFile}"` : shellQuote(runnerFile),
      "tab",
    );

    writeFileSecure(
      join(pipelineDir, "pipeline.meta.json"),
      JSON.stringify(
        {
          pipeline_id: pipelineId,
          directory,
          total_steps: normalizedTasks.length,
          tasks: normalizedTasks.map((t, i) => ({
            step: i,
            name: t.name,
            model: t.model,
          })),
...
export function handleGetPipeline(args) {
  ...
  if (!existsSync(pipelineDir))
    return text(`Pipeline ${pipeline_id} not found.`);
  ...
  if (existsSync(logFile)) {
    logEntries = readFileSync(logFile, "utf-8")
      .trim()
      .split("\n")
      .map((l) => {
        try {
