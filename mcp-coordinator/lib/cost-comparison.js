...
export function handleCostComparison() {
  ...
  const agentTeamsWorkerCost =
    workerCount > 0
      ? workers.reduce((sum, w) => {
          const tokens =
            (w.estimated_tokens || w.tokens || 80000) * TEAMMATE_OVERHEAD;
  ...
  const agentTeamsTotal =
    agentTeamsSessionCost + agentTeamsWorkerCost + agentTeamsCoordCost;

  const savings = agentTeamsTotal - leadTotal;
  const savingsPct =
    agentTeamsTotal > 0 ? ((savings / agentTeamsTotal) * 100).toFixed(0) : 0;
