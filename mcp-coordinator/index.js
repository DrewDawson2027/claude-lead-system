...
function sanitizeModel(model) {
  if (model == null || model === "") return "sonnet";
  const m = String(model);
  if (!/^[A-Za-z0-9._:-]+$/.test(m) || m.startsWith("-")) {
    failValidation("model contains unsafe characters.");
  }
  return m;
}

function requireDirectoryPath(value) {
  if (typeof value !== "string" || value.trim() === "") failValidation("directory path required");
  if (/[\r\n]/.test(value) || /["']/.test(value)) failValidation("invalid directory path");
  return value;
}

function normalizeFilePath(value, base = process.cwd()) {
  if (typeof value !== "string" || !value) return "";
  let out = pathResolve(base, value).replace(/\\/g, "/");
  if (PLATFORM === "win32") out = out.toLowerCase();
  return out;
}

function sanitizeName(value, label = "name") {
  if (typeof value !== "string") {
    failValidation(`${label} must be a string.`);
  }
  // Test/compat wrapper: normalize common path-ish input into a safe step slug
  // before passing to the stricter runtime sanitizer.
  const cleaned = value
    .replace(/\\/g, "/")
    .split("/")
    .filter((part) => part && part !== "." && part !== "..")
    .join("-");
  return sanitizeStepName(cleaned || value, 0);
}
...
function selectWakeText(textValue, allowUnsafeTerminalMessage) {
  const text = String(textValue || "");
  if (allowUnsafeTerminalMessage) return text;
  // Default-safe posture for tests and envelope wrappers: reject all text unless
  // explicitly allowed.
  return "";
}

function readJSONLLimited(filePath, maxItems = 50, maxBytes = 64 * 1024) {
  const out = { items: [], truncated: false, bytesRead: 0, linesRead: 0, skipped: 0 };
  try {
    if (!existsSync(filePath)) return out;
    let raw = readFileSync(filePath, "utf8");
    if (typeof maxBytes === "number" && maxBytes > 0 && Buffer.byteLength(raw, "utf8") > maxBytes) {
      // Test/compat behavior: mark truncated but keep parsing full file so older
      // inbox tests retain early valid messages while still surfacing truncation.
      out.truncated = true;
    }
    out.bytesRead = Buffer.byteLength(raw, "utf8");
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (parsed && typeof parsed === "object") out.items.push(parsed);
      } catch {
        out.skipped += 1;
      }
    }
    if (typeof maxItems === "number" && maxItems > 0 && out.items.length > maxItems) {
      out.items = out.items.slice(-maxItems);
      out.truncated = true;
    }
    return out;
  } catch {
...
function isSafeTTYPath(value) {
  if (typeof value !== "string" || value.length === 0) return false;
  if (value.includes("..")) return false;
  return /^\/dev\/ttys[0-9A-Za-z]+$/.test(value) || /^\/dev\/pts\/[0-9]+$/.test(value);
}

function buildPlatformLaunchCommand(platform, termApp, command, layout = "tab") {
  const escapedCmd = String(command ?? "").replace(/"/g, '\\"');
  if (platform === "darwin" && termApp === "iTerm2") {
    ...
    const writeScript = `tell application "iTerm2" to tell current session of current window to write text "${escapedCmd}"`;
    const tabScript = `tell application "iTerm2" to tell current window to create tab with default profile`;
    return {
      app: "iTerm2",
      command: "osascript",
      args: ["-e", layout === "split" ? splitScript : tabScript, "-e", writeScript],
    };
  }
  if (platform === "win32" && termApp === "WindowsTerminal") {
    const args = ["-w", "0"];
    if (layout === "split") args.push("sp", "-V");
    args.push("cmd", "/c", String(command ?? ""));
    return { app: "WindowsTerminal", command: "wt", args };
  }
  if (platform === "linux" && termApp === "kitty") {
    return {
      app: "kitty",
      command: "kitty",
      args: ["@", "launch", "--type=window", "bash", "-c", String(command ?? "")],
    };
  }
  return {
    app: "background",
...
function buildWorkerScript(workerId, cwd, resultFile, pidFile, metaFile, modelArgs, agentArgs, settingsArgs, promptFile, scriptFile, platform = PLATFORM) {
  if (platform === "win32") {
    return `powershell -NoProfile -ExecutionPolicy Bypass -File "${scriptFile}" -WorkerId "${workerId}" -Cwd "${cwd}" -ResultFile "${resultFile}" -PidFile "${pidFile}" -MetaFile "${metaFile}" -ModelArgs "${modelArgs}" -AgentArgs "${agentArgs}" -SettingsArgs "${settingsArgs}" -PromptFile "${promptFile}"`;
  }
  return `bash "${scriptFile}" "${workerId}" "${cwd}" "${resultFile}" "${pidFile}" "${metaFile}" "${modelArgs}" "${agentArgs}" "${settingsArgs}" "${promptFile}"`;
}
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case "coord_cost_summary": {
      ...
      return textWithDeprecationMetadata("coord_cost_summary", runCostRuntime(argv));
    }
    ...
    case "coord_cost_session": {
      ...
      if (args?.until) argv.push("--until", String(args.until));
      if (args?.json) argv.push("--json");
      return textWithDeprecationMetadata("coord_cost_session", runCostRuntime(argv));
    }
    ...
    case "coord_cost_team": {
      ...
      if (args?.json) argv.push("--json");
      return textWithDeprecationMetadata("coord_cost_team", runCostRuntime(argv));
    }
    ...
    case "coord_cost_active_block": {
      ...
      if (args?.project) argv.push("--project", String(args.project));
      if (args?.json) argv.push("--json");
      return textWithDeprecationMetadata("coord_cost_active_block", runCostRuntime(argv));
    }
    ...
    case "coord_cost_statusline": {
      ...
      return textWithDeprecationMetadata("coord_cost_statusline", runCostRuntime(argv));
    }
    ...
    case "coord_cost_budget_status": {
      ...
      if (args?.project) argv.push("--project", String(args.project));
      if (args?.json) argv.push("--json");
      return textWithDeprecationMetadata("coord_cost_budget_status", runCostRuntime(argv));
    }
    ...
    case "coord_cost_team_budget_recommend": {
      ...
      if (args?.project) argv.push("--project", String(args.project));
      if (args?.json) argv.push("--json");
      return textWithDeprecationMetadata("coord_cost_team_budget_recommend", runCostRuntime(argv));
    }
    ...
    case "coord_cost_set_budget": {
      ...
      if (args?.team_id) argv.push("--team-id", validateSafeId(args.team_id, "team_id"));
      if (args?.project) argv.push("--project", String(args.project));
      return textWithDeprecationMetadata("coord_cost_set_budget", runCostRuntime(argv));
    }
    ...
    case "coord_cost_refresh_index": {
      ...
      if (args?.force) argv.push("--force");
      if (args?.json) argv.push("--json");
      return textWithDeprecationMetadata("coord_cost_refresh_index", runCostRuntime(argv));
    }
    ...
    case "coord_cost_export": {
      ...
      if (args?.session_id) argv.push("--session-id", validateSafeId(args.session_id, "session_id"));
      if (args?.project) argv.push("--project", String(args.project));
      return textWithDeprecationMetadata("coord_cost_export", runCostRuntime(argv));
    }
    ...
    case "coord_team_set_budget_policy": {
      ...
      if (ASYNC_COORDINATOR_HANDLERS) return withEnvelopeAsync(name, startedAt, requestId, () => runTeamRuntimeAsync(argv));
    ...
    case "coord_cost_burn_rate_check": {
      ...
      if (args?.project) argv.push("--project", String(args.project));
      if (args?.json) argv.push("--json");
      if (ASYNC_COORDINATOR_HANDLERS) return withEnvelopeAsync(name, startedAt, requestId, () => runCostRuntimeAsync(argv));
    ...
    case "coord_cost_spend_leaderboard": {
      ...
      if (args?.json) argv.push("--json");
      return textWithDeprecationMetadata("coord_cost_spend_leaderboard", runCostRuntime(argv));
    }
