...
# ─────────────────────────────────────────────────────────────────────────────
# 8. Native Action Queue Bridge — integration test
# ─────────────────────────────────────────────────────────────────────────────


class TestNativeActionQueueBridge:
    """Integration test for the filesystem-based native action queue bridge.

    The bridge writes JSON action files to lead-sidecar/runtime/actions/pending/.
    The lead session drains them via handleDrainNativeQueue. This tests the
    filesystem contract that both sides agree on.
    """

    @pytest.fixture
    def queue_env(self, tmp_path):
        """Create isolated action queue directory structure."""
        actions_dir = tmp_path / "lead-sidecar" / "runtime" / "actions" / "pending"
        actions_dir.mkdir(parents=True)
        done_dir = actions_dir.parent / "done"
        done_dir.mkdir(parents=True)
        return actions_dir, done_dir

    def _write_action(self, actions_dir, action, filename=None):
        """Write a native action file to the queue."""
        import time

        fname = filename or f"msg-{int(time.time() * 1000)}.json"
        path = actions_dir / fname
        path.write_text(json.dumps(action, indent=2))
        return path

    def test_action_file_schema(self, queue_env):
        """Action files have required fields: ts, action, recipient, content."""
        actions_dir, _ = queue_env
        action = {
        ...
        path = self._write_action(actions_dir, action)
        loaded = json.loads(path.read_text())
        assert loaded["action"] == "native_send_message"
        assert loaded["recipient"] == "worker-alpha"
        assert loaded["content"] == "Build the auth module"
        assert loaded["delivery"] == "native_push"

    def test_ttl_expiry_removes_old_files(self, queue_env):
        """Files older than 5 minutes should be considered expired."""
        actions_dir, _ = queue_env
        import time
        ...
        old_action = {
            ...
            "action": "native_send_message",
            "recipient": "stale-worker",
            "content": "old message",
        }
        path = self._write_action(actions_dir, old_action, "msg-old.json")
        old_mtime = time.time() - (6 * 60)
        os.utime(path, (old_mtime, old_mtime))

        TTL_MS = 5 * 60 * 1000
        age_ms = (time.time() - path.stat().st_mtime) * 1000
        assert age_ms > TTL_MS, "File should be older than 5-minute TTL"

        fresh_action = {
            "ts": "2026-03-06T00:00:00.000Z",
            "action": "native_send_message",
            "recipient": "active-worker",
            "content": "fresh message",
        }
        fresh_path = self._write_action(actions_dir, fresh_action, "msg-fresh.json")
        fresh_age_ms = (time.time() - fresh_path.stat().st_mtime) * 1000
        assert fresh_age_ms < TTL_MS, "Fresh file should be within TTL"

    def test_max_queue_depth_enforced(self, queue_env):
        """Queue should not accept more than 50 action files."""
        actions_dir, _ = queue_env
        MAX_QUEUE_DEPTH = 50
        ...
        assert len(files) == MAX_QUEUE_DEPTH + 5

    def test_done_directory_exists_for_processed_actions(self, queue_env):
        """After processing, actions move to done/ subdirectory."""
        actions_dir, done_dir = queue_env
        action = {
            "ts": "2026-03-06T00:00:00.000Z",
            "action": "native_send_message",
        ...
        path = self._write_action(actions_dir, action, "msg-delivery.json")
        dest = done_dir / path.name
        path.rename(dest)
        assert dest.exists()
        assert not path.exists()
        loaded = json.loads(dest.read_text())
        assert loaded["recipient"] == "worker-beta"

    def test_session_id_vs_name_routing(self, queue_env):
        """Action queue routes by session ID (8-char hex) vs worker name."""
        actions_dir, _ = queue_env
        import re
        ...
        sid_action = {
            "action": "native_send_message",
            "recipient": "a1b2c3d4",
            "content": "by session ID",
        }
        path_sid = self._write_action(actions_dir, sid_action, "msg-sid.json")
        loaded = json.loads(path_sid.read_text())
        is_session_id = bool(re.match(r"^[0-9a-f]{8}$", loaded["recipient"], re.I))
        assert is_session_id, "8-char hex should be detected as session ID"

        name_action = {
            "action": "native_send_message",
            "recipient": "worker-alpha",
            "content": "by worker name",
        }
        path_name = self._write_action(actions_dir, name_action, "msg-name.json")
        loaded = json.loads(path_name.read_text())
        is_name = not bool(re.match(r"^[0-9a-f]{8}$", loaded["recipient"], re.I))
        assert is_name, "Non-hex string should be detected as worker name"

    def test_malformed_action_file_handled(self, queue_env):
        """Corrupted JSON files should not crash the drain process."""
        actions_dir, _ = queue_env
        bad_path = actions_dir / "msg-corrupt.json"
