...
export function registerMaintenanceRoutes(registry: any): void {
  ...
  registry.add("maintenance:checkpoints-create", async (ctx: any) => {
    ...
    try {
      const result = ctx.createCheckpoint(
        paths,
        String(body.label || "manual"),
      );
  ...
  registry.add("maintenance:checkpoints-list", (ctx: any) => {
    ...
    ctx.sendJson(
      res,
      200,
      { ok: true, checkpoints: ctx.listCheckpoints(paths) },
      req,
    );
    return true;
  });
  ...
  registry.add("maintenance:checkpoints-restore", async (ctx: any) => {
    ...
    if (!body.file) {
      ctx.sendError(res, 400, "VALIDATION_ERROR", "file is required", req);
      return true;
    }
    ...
    if (!isPathWithin(paths.checkpointsDir, resolved, ctx.pathResolve)) {
      ctx.sendError(
        ...
        400,
        "VALIDATION_ERROR",
        "file must be within checkpoints directory",
        req,
      );
      return true;
    }
  ...
  registry.add("maintenance:repair-fix", async (ctx: any) => {
    ...
    if (!body.path) {
      ctx.sendError(res, 400, "VALIDATION_ERROR", "path is required", req);
      return true;
    }
    const resolved = ctx.pathResolve(String(body.path));
    if (
      !isPathWithin(paths.root, resolved, ctx.pathResolve) &&
      !isPathWithin(paths.terminalsDir, resolved, ctx.pathResolve)
    ) {
      ctx.sendError(
        res,
        400,
        "VALIDATION_ERROR",
        "path must be within sidecar or terminals directory",
        req,
      );
      return true;
    }
    if (body.dry_run) {
      try {
        ...
        ctx.sendJson(
          res,
          200,
          { ok: true, dry_run: true, valid: data !== null, path: resolved },
          req,
        );
      } catch (err: any) {
        ctx.sendJson(
    ...
    const result = resolved.endsWith(".jsonl")
      ? ctx.repairJSONL(resolved)
      : ctx.repairJSON(resolved);
    ctx.sendJson(res, 200, { ok: true, ...result }, req);
    return true;
  });
  ...
  registry.add("maintenance:schema-migrations", (ctx: any) => {
    ...
    ctx.sendJson(
      ...
      {
        ok: true,
        current_version: ctx.CURRENT_SCHEMA_VERSION,
        migrations: ctx.migrations.map((m: any) => ({
          from: m.from,
          to: m.to,
  ...
  registry.add("maintenance:backups-list", (ctx: any) => {
    ...
    ctx.sendJson(
      res,
      200,
      { ok: true, backups: ctx.listBackups(paths, operation) },
      req,
    );
    return true;
  });
  ...
  registry.add("maintenance:backups-restore", async (ctx: any) => {
    ...
    if (!isPathWithin(paths.backupsDir, resolved, ctx.pathResolve)) {
      ctx.sendError(
        ...
        400,
        "VALIDATION_ERROR",
        "file must be within backups directory",
        req,
      );
      return true;
    }
    ...
    ctx.sendJson(
      res,
      result.restored ? 200 : 400,
      { ok: result.restored, ...result },
      req,
    );
    return true;
  });
  ...
  registry.add("maintenance:request-audit", (ctx: any) => {
    const { req, res, url } = ctx;
    if (!(req.method === "GET" && url.pathname === "/health/request-audit"))
      return false;
    const snapshot = ctx.requestAuditLog?.snapshot() || {
      total: 0,
      recent: [],
      by_method: {},
      by_status: {},
  ...
  registry.add("maintenance:security-audit", (ctx: any) => {
    const { req, res, url } = ctx;
    if (!(req.method === "GET" && url.pathname === "/health/security-audit"))
      return false;
    const snapshot = ctx.securityAuditLog?.snapshot() || {
      total: 0,
      recent: [],
      by_type: {},
    };
  ...
  registry.add("maintenance:rotate-api-token", async (ctx: any) => {
    ...
    if (
      !(
        req.method === "POST" &&
        url.pathname === "/maintenance/rotate-api-token"
      )
    )
      return false;
    if (SAFE_MODE) {
    ...
    ctx.sendJson(
      res,
      200,
      { ok: true, new_token: result.new_token, rotated_at: result.rotated_at },
      req,
    );
    return true;
  });
