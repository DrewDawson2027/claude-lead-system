...
export function registerNativeRoutes(registry: any): void {
  registry.add('native:status', async (ctx: any) => {
    ...
    if (!(req.method === 'GET' && url.pathname === '/native/status')) return false;
    const native = await ctx.nativeAdapter.getStatus().catch((err: any) => ({ adapter_ok: false, error: err.message, mode: 'unavailable' }));
    ctx.store.setNativeCapabilities({ ...(native.native || { available: false, last_probe_error: native.error || null }), validation: native.bridge_validation || null });
    if (native.bridge) ctx.store.emitBridgeStatus(native.bridge);
    ctx.store.setSnapshot({ native: ctx.store.getSnapshot().native, actions: ctx.store.getSnapshot().actions, alerts: ctx.store.getSnapshot().alerts, metrics: ctx.store.getSnapshot().metrics });
  ...
  registry.add('native:bridge-status', (ctx: any) => {
    const { req, res, url } = ctx;
    if (!(req.method === 'GET' && url.pathname === '/native/bridge/status')) return false;
  ...
  registry.add('native:bridge-validation', async (ctx: any) => {
    const { req, res, url } = ctx;
    if (!(req.method === 'GET' && url.pathname === '/native/bridge/validation')) return false;
    const native = await ctx.nativeAdapter.getStatus().catch((err: any) => ({ adapter_ok: false, error: err.message, mode: 'unavailable' }));
    ctx.sendJson(res, 200, { ok: true, validation: native.bridge_validation || null, bridge: native.bridge || null }, req);
    return true;
  });
  ...
  registry.add('native:action', async (ctx: any) => {
    ...
    const out = await ctx.runTrackedAction({ team, action, payload: body, routeMode: 'native-direct', nativeHttpAction: action });
