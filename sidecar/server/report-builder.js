...
export function buildComparisonReport(bundle, opts = {}) {
  ...
  const successRate = total > 0 ? ((counts.success || 0) / total * 100).toFixed(1) : '—';
  const fallbackRate = total > 0 ? ((counts.fallback || 0) / total * 100).toFixed(1) : '—';
  ln(`| Metric | Value |`);
  ln(`|--------|-------|`);
  ...
  const staleCount = teammates.filter(t => t.presence === 'stale' || t.presence === 'offline').length;
  ln(`- **Teams**: ${teams.length}`);
  ln(`- **Members**: ${teammates.length} (${staleCount} stale/offline)`);
  ln(`- **Tasks**: ${tasks.length} (${tasks.filter(t => t.status === 'pending').length} pending, ${tasks.filter(t => t.status === 'in_progress').length} in-progress)`);
  if (teammates.length > 0) {
    const loads = teammates.map(t => t.load_score ?? 0);
    const avgLoad = (loads.reduce((a, b) => a + b, 0) / loads.length).toFixed(1);
    const maxLoad = Math.max(...loads);
    const minLoad = Math.min(...loads);
    ln(`- **Load**: avg=${avgLoad}, min=${minLoad}, max=${maxLoad}, spread=${maxLoad - minLoad}`);
  }
  blank();
  ...
  // Bridge Health
  h(2, 'Bridge Health');
  const bridgeStatus = native.status?.bridge_status || native.status?.status || '—';
  ...
  if (validation) {
    ln(`- **Validation**: ${validation.valid ? 'PASS' : 'FAIL'} (${validation.checks_passed ?? '?'}/${validation.checks_total ?? '?'} checks)`);
  }
  blank();
  ...
  if (baseline) {
    ...
    ln(`|--------|----------|---------|-------|`);
    const delta = (a, b) => b != null && a != null ? `${(b - a) > 0 ? '+' : ''}${(b - a).toFixed(2)}` : '—';
    ln(`| Latency p50 | ${baseMetrics.action_latency_ms?.p50?.toFixed(2) ?? '—'}ms | ${latP50?.toFixed(2) ?? '—'}ms | ${delta(baseMetrics.action_latency_ms?.p50, latP50)} |`);
    ln(`| Latency p95 | ${baseMetrics.action_latency_ms?.p95?.toFixed(2) ?? '—'}ms | ${latP95?.toFixed(2) ?? '—'}ms | ${delta(baseMetrics.action_latency_ms?.p95, latP95)} |`);
    ln(`| Actions | ${baseTotal} | ${curTotal} | ${delta(baseTotal, curTotal)} |`);
    ln(`| Failures | ${baseCounts.failure || 0} | ${counts.failure || 0} | ${delta(baseCounts.failure || 0, counts.failure || 0)} |`);
    blank();
  }
  ...
  const json = {
    ...
    performance: { latency_p50: latP50, latency_p95: latP95, success_rate: successRate, fallback_rate: fallbackRate, total_actions: total },
    capacity: { teams: teams.length, members: teammates.length, stale: staleCount, tasks: tasks.length },
