...
export function buildActionPayload(
  ...
  if (action === "wake")
    return {
      ...
      message: body.message || "Lead sidecar wake request",
      team_name: teamName,
    };
...
export function buildTeamInterrupts({
  ...
  for (const a of alerts) {
    interrupts.push({
      ...
      suggested_actions:
        a.code === "bridge_stuck_request"
          ? ["bridge-validate", "bridge-ensure"]
          : ["view-action"],
      safe_auto: false,
  ...
  return interrupts.map((i, idx) => ({
    ...i,
    priority_score: interruptPriority(i.code, i.severity, weights),
    rank: idx + 1,
  }));
}
