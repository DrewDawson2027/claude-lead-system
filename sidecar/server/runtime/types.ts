...
export type MkdirSyncFn = (path: string, opts?: { recursive?: boolean }) => void;
...
export interface RuntimeActionQueue {
    create(opts: Record<string, unknown>): { action_id: string; team_name?: string | null;[k: string]: unknown };
    ...
    sweep(opts: Record<string, number>): { pending: number; done: number; failed: number; requests?: number; responses?: number };
...
export interface RuntimeMetrics {
    snapshot(): Record<string, unknown>;
    observeAction(data: { latency_ms: number; path_key: string; ok: boolean; fallback_used: boolean }): void;
...
export interface RuntimeActionRouter {
    route(team: Record<string, unknown>, action: string, payload: Record<string, unknown>): Promise<Record<string, unknown>>;
}
...
export interface RuntimeRateLimiter {
    check(key: string): { ok: boolean; remaining?: number; retry_after_ms?: number };
    gc(): void;
}
...
export interface MaintenanceSweepDeps {
    ...
    paths: SidecarPaths;
    findStuckBridgeRequests: (paths: SidecarPaths, thresholdMs: number) => Array<any>;
    sweepBridgeQueues: (paths: SidecarPaths, opts: { requestMaxAgeMs: number; responseMaxAgeMs: number }) => { requests: number; responses: number };
    store: RuntimeStore;
    rateLimiter: RuntimeRateLimiter;
    getAllTasksSnapshot: () => any;
    applyPriorityAging: (tasks: any[], opts: Record<string, unknown>) => { aged: string[]; tasks: Array<{ task_id: string; priority: string; metadata: unknown }> };
    getTeamsSnapshot: (fresh?: boolean) => any;
    shouldAutoRebalance: (teamSnap: any, config: any) => { trigger: boolean; reason?: string; conditions_met?: unknown;[k: string]: any };
    ...
    checkTerminalHealth: (paths: SidecarPaths) => { summary: string; zombies: any[]; stale: any[]; dead_shells: any[];[k: string]: any };
    suggestRecovery: (health: any) => any[];
    validateHooks: (hooksDir: string) => { all_valid: boolean; hooks: Array<{ name: string; issues: any[] }> };
}
...
export interface BatchTriageDeps {
    ...
    buildTeamInterrupts: (opts: { snapshot: unknown; teamName: string; teamPolicy?: unknown }) => Array<Record<string, unknown>>;
    runTrackedAction: (input: TrackedActionInput) => Promise<Record<string, unknown>>;
}
...
export interface HookStreamAdapterLike {
    new(paths: SidecarPaths, onChange: (evt: Record<string, unknown>) => void): {
        start(): void;
        stop(): void;
    };
}
...
export interface RuntimeLifecycleDeps {
    ...
    sseBroadcast: (clients: Set<http.ServerResponse>, event: string, data: unknown) => void;
}
