    ...
    try {
      const routed =
        routeMode === "native-direct"
          ...
          : await router.route(team, action, {
              ...payload,
      ...
      if (ok) {
        actionQueue.markCompleted(record.action_id, {
          ...
          latency_ms: wrapper.latency_ms,
          result_summary: wrapper.result?.text
...
export function createBatchTriageRunner({
  ...
  return async function runBatchTriage({
    ...
    if (op === "approve_all_safe") {
      selected = interrupts
        .filter((i) => i.kind === "approval" && i.safe_auto)
        .slice(0, max);
      ...
      selected = interrupts
        .filter((i) => i.kind === "stale" && i.safe_auto && i.session_id)
        .slice(0, max);
      for (const it of selected) {
        try {
          const t = findTeam(store.getSnapshot(), teamName);
