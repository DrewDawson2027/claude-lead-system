...
export function sameOriginAllowed(req: any, allowedOrigin: string | null = null, allowedOrigins: string[] = []): boolean {
...
export function createBaseHeaders(req: any = null, allowedOrigin: string | null = null, allowedOrigins: string[] = []): Record<string, string> {
  ...
  if (origin && sameOriginAllowed(req, allowedOrigin, allowedOrigins)) {
    ...
    headers.Vary = 'Origin';
    headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, X-Sidecar-CSRF, X-Sidecar-Nonce';
...
export function sendError(baseHeaders: (req?: any) => Record<string, string>, res: any, status: number, errorCode: string, message: string, req: any = null, details?: unknown): void {
