...
export function createLogger({ format = 'text' as LogFormat } = {}) {
  ...
  return {
    ...
    request: (req: { method?: string; url?: string; __requestId?: string; socket?: { remoteAddress?: string } }, status: number, startMs: number) => {
      const duration_ms = Date.now() - startMs;
      const method = String(req.method || 'GET');
      const path = String(req.url || '/');
      const request_id = String((req as any).__requestId || '-');
      const ip = String(req.socket?.remoteAddress || 'unknown');
      emit('info', `${method} ${path} ${status} ${duration_ms}ms`, { method, path, status, duration_ms, request_id, ip });
    },
  };
}
