...
export function requireCsrf(
  ...
  const csrfPayload: Record<string, unknown> = {
  ...
  return {
    check(key: string) {
      ...
      if (b.count > max) {
        return {
          ok: false,
          retry_after_ms: Math.max(0, windowMs - (now - b.start)),
        };
      }
...
const REPLAY_PROTECTED_ROUTES = [
  /^\/dispatch$/,
  /^\/teams\/[^/]+\/actions\//,
  /^\/maintenance\/rotate-api-token$/,
];
...
export function createReplayProtector({
  ...
  return {
    check(
      ...
      if (process.env.LEAD_SIDECAR_REPLAY_PROTECTION !== "1")
        return { ok: true };
      const nonce = String(req.headers["x-sidecar-nonce"] || "");
      if (!nonce) return { ok: true };
      if (!REPLAY_PROTECTED_ROUTES.some((rx) => rx.test(pathname)))
        return { ok: true };
      const now = Date.now();
      // GC expired nonces
