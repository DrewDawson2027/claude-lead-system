...
export function diffSnapshots(before, after) {
  ...
  const beforeMembers = new Map(
    (before.teammates || []).map((m) => [m.id || m.display_name, m]),
  );
  const afterMembers = new Map(
    (after.teammates || []).map((m) => [m.id || m.display_name, m]),
  );
  for (const [id, m] of afterMembers) {
    if (!beforeMembers.has(id)) {
      added.push({ type: "teammate", id, detail: m });
      continue;
    }
    const prev = beforeMembers.get(id);
    const changes = [];
    if (prev.presence !== m.presence)
      changes.push({
        field: "presence",
        before: prev.presence,
        after: m.presence,
      });
    if (prev.load_score !== m.load_score)
      changes.push({
        field: "load_score",
        before: prev.load_score,
        after: m.load_score,
      });
  ...
  const beforeTasks = new Map(
    (before.tasks || []).map((t) => [t.task_id || t.id, t]),
  );
  const afterTasks = new Map(
    (after.tasks || []).map((t) => [t.task_id || t.id, t]),
  );
  for (const [id, t] of afterTasks) {
    if (!beforeTasks.has(id)) {
      added.push({
        type: "task",
        id,
        detail: { subject: t.subject, status: t.status },
      });
      continue;
    }
    ...
    if (prev.status !== t.status)
      changes.push({ field: "status", before: prev.status, after: t.status });
    if (prev.assignee !== t.assignee)
      changes.push({
        field: "assignee",
        before: prev.assignee,
        after: t.assignee,
      });
  ...
  if (beforeAlertCount !== afterAlertCount) {
    changed.push({
      ...
      id: "count",
      changes: [
        { field: "count", before: beforeAlertCount, after: afterAlertCount },
      ],
    });
  }
