...
export function createCheckpoint(paths, label = "manual") {
  ...
  const file = join(
    ...
    `cp-${Date.now()}-${label.replace(/[^a-zA-Z0-9_-]/g, "_")}.json`,
  );
...
export function restoreCheckpoint(paths, checkpointFile) {
  ...
  if (!cp)
    return {
      ...
      teams_count: 0,
      tasks_count: 0,
      error: "checkpoint file unreadable",
    };
  ...
  if (cp.snapshot) {
    try {
      writeJSON(paths.snapshotFile, cp.snapshot);
    } catch {
      /* continue */
    }
  }
...
export function rotateCheckpoints(paths, maxCount = 20) {
  ...
  for (const f of toRemove) {
    try {
      unlinkSync(join(paths.checkpointsDir, f));
      removed++;
    } catch {
      /* skip */
    }
