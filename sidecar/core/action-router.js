...
export class ActionRouter {
  ...
  async route(team, action, payload = {}) {
    ...
    const decision = chooseExecutionPath(team, action, nativeHealth, payload || {});
    const started = Date.now();
    try {
      const primaryResult = decision.adapter === 'native'
        ? await this.native.execute(action, payload, { team, force_path_mode: decision.path_mode })
      ...
      const response = {
        ...
        path_mode: decision.adapter === 'native' ? (primaryResult?.path_mode || decision.path_mode) : 'local-module',
      ...
      if (!ok && decision.adapter === 'native' && (team?.policy?.native_fallback_policy || 'coordinator') !== 'error') {
      ...
      if (decision.adapter === 'native' && (team?.policy?.native_fallback_policy || 'coordinator') !== 'error') {
        const fallback = await this.coordinator.execute(action, payload);
        return {
          ok: true,
          adapter: 'coordinator',
          ...
          latency_ms: Date.now() - started,
          result: fallback,
          fallback_from: { adapter: 'native', path_mode: decision.path_mode, error: { message: err.message } },
        };
      }
      return {
        ok: false,
        adapter: decision.adapter,
        path_mode: decision.adapter === 'native' ? decision.path_mode : 'local-module',
        reason: decision.reason,
        fallback_plan: decision.fallback_plan,
        fallback_used: false,
        cost_estimate_class: decision.cost_estimate_class,
