...
export class SidecarStateStore extends EventEmitter {
  ...
  _recordMetric(name, value = 1) {
    if (Object.prototype.hasOwnProperty.call(this.persistenceMetrics, name)) {
      ...
      this.persistenceMetrics[name] =
        (this.persistenceMetrics[name] || 0) + value;
    }
    if (this.metricSink) {
      try {
        this.metricSink(name, value);
      } catch (err) {
        void err;
      }
    }
    ...
    while (true) {
      try {
        ...
        this._log(
          logLevel,
          alertMessage,
          this._safeMeta({ ...meta, error_code, message }),
        );
        return {
          ok: false,
          error_code,
  ...
  setSnapshot(next) {
    ...
    return {
      ok: false,
      error_code: !snapshotWrite.ok
        ? snapshotWrite.error_code
        : timelineAppend.error_code,
      message: !snapshotWrite.ok
        ? snapshotWrite.message
        : timelineAppend.message,
  ...
  emitBridgeStatus(status) {
    this.snapshot.native = {
      ...(this.snapshot.native || {}),
      bridge: status,
      bridge_status: status.bridge_status || status.status || null,
    };
    this.emit("native.bridge.status", {
      ts: new Date().toISOString(),
      ...status,
    });
  }
  ...
  raiseAlert(alert) {
    ...
    const alerts = Array.isArray(this.snapshot.alerts)
