...
import { readJSON, readText, listDir } from "./fs-utils.js";
...
export function checkTerminalHealth(paths) {
  ...
  // Check session files
  for (const f of listDir(paths.terminalsDir).filter(
    ...
    const sessionId =
      session.session_id || f.replace("session-", "").replace(".json", "");
    ...
    if (pid && !isProcessAlive(pid)) {
      zombies.push({
        ...entry,
        issue: "PID not alive but session file still active",
      });
    } else if (
      ...
      stale.push({
        ...entry,
        issue: `No update for ${(ageMs / 60_000).toFixed(1)} minutes`,
      });
    } else {
      healthy.push(entry);
    }
  ...
  for (const f of listDir(resultsDir).filter(
    (x) => x.endsWith(".meta.json") && !x.includes(".done"),
  )) {
    ...
    // Check if worker PID is still alive (.pid files are plain text, not JSON)
    const pidRaw = readText(pidFile);
    const parsedPid = pidRaw ? parseInt(pidRaw.trim(), 10) : NaN;
    const pid =
      Number.isFinite(parsedPid) && parsedPid > 0 ? parsedPid : meta.pid;
    if (pid && !isProcessAlive(pid)) {
      dead_shells.push({
        task_id: taskId,
        file: f,
