  ...
  for (const e of events) {
    ...
    if (lastTs) {
      ...
      if (ts - lastTs > 5 * 60_000) {
        gaps.push(
          `${new Date(lastTs).toISOString()} → ${new Date(ts).toISOString()} (${((ts - lastTs) / 60_000).toFixed(1)}min)`,
        );
      }
      lastTs = ts;
    } else if (e.ts) {
    ...
    switch (type) {
      case "snapshot.rebuilt":
        derived.snapshots_rebuilt++;
        break;
      case "snapshot":
        derived.snapshots_rebuilt++;
        break;
      case "task.created":
        derived.tasks_created++;
        break;
      case "task.updated":
        derived.tasks_updated++;
        break;
      case "message.sent":
      case "message.delivered":
        ...
        break;
      case "alert.raised":
        derived.alerts_raised++;
        break;
      case "maintenance.sweep":
        derived.maintenance_sweeps++;
        break;
      case "auto_rebalance.triggered":
        derived.auto_rebalances++;
        break;
    }
...
export function consistencyCheck(derived, actual) {
  ...
  for (const t of derivedTeams) {
    if (!actualTeams.includes(t)) {
      diffs.push({
        ...
        expected: t,
        actual: "missing",
        issue: "Team seen in events but not in current snapshot",
      });
    }
  }
  ...
  if (derived.derived_state.alerts_raised > 0 && actualAlerts === 0) {
    diffs.push({
      field: "alerts",
      expected: `>0 (${derived.derived_state.alerts_raised} raised)`,
      actual: 0,
      issue: "Alerts raised in events but none in snapshot",
    });
  }

  // Check that snapshot was rebuilt at least once
  if (
    derived.derived_state.snapshots_rebuilt === 0 &&
    derived.event_count > 10
  ) {
    diffs.push({
      field: "snapshots_rebuilt",
      expected: ">0",
      actual: 0,
      issue: "No snapshot rebuild events found despite activity",
    });
  }
