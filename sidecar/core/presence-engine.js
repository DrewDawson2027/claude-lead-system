export function derivePresenceFromMember(member = {}, allTasks = []) {
  ...
  const risks = Array.isArray(member.risk_flags)
    ? [...new Set(member.risk_flags)]
    : [];
  ...
  if (sessionStatus === "stale")
    return { presence: "stale", risk_flags: risks };
  if (sessionStatus === "idle") return { presence: "idle", risk_flags: risks };
  if (task?.blocked_by?.length)
    return { presence: "blocked_by_dependency", risk_flags: risks };
