...
export class CoordinatorAdapter {
  async health() {
    return { ok: true, mode: 'local-module', note: 'Coordinator handler modules loaded directly.' };
  }
  ...
  async execute(action, payload = {}) {
    switch (action) {
      case 'queue-task': return { text: toText(handleTeamQueueTask(payload)) };
      case 'assign-next': return { text: toText(handleTeamAssignNext(payload)) };
      case 'rebalance': return { text: toText(handleTeamRebalance(payload)) };
      case 'rebalance-explain': return this.rebalanceExplain(payload.team_name, payload);
      case 'dispatch': return { text: toText(handleTeamDispatch(payload)) };
      case 'message': return { text: toText(handleSendMessage(payload)) };
