...
function settingsReader(paths) {
  return () => readJSON(paths.settingsFile || `${paths.claudeDir}/settings.local.json`) || {};
}
...
export class NativeTeamAdapter {
  constructor({ paths = sidecarPaths(), coordinatorAdapter = null, store = null, runner = null, bridgeController = null } = {}) {
    this.paths = paths;
    this.store = store;
    this.coordinator = coordinatorAdapter;
    this.runner = runner || new NativeActionRunner({});
    this.bridge = bridgeController || new BridgeController({ paths, coordinatorAdapter, store });
  ...
  async probe({ force = true } = {}) {
    const caps = await this.detector.detect({ force, ttlMs: this._probeTtlMs(), team: { execution_path: 'hybrid' } });
    this.store?.setNativeCapabilities?.(caps);
    return caps;
  }

  _probeTtlMs(team = {}) {
    const ttlSeconds = Number(team?.policy?.native_probe_ttl_seconds || process.env.LEAD_SIDECAR_NATIVE_PROBE_TTL_SECONDS || 60);
    return Math.max(5, Math.min(3600, Number.isFinite(ttlSeconds) ? ttlSeconds : 60)) * 1000;
  }

  async health(team = {}) {
    const caps = await this.detector.detect({ force: false, ttlMs: this._probeTtlMs(team), team });
    const bridge = this.bridge.getHealth();
    const out = {
      ok: Boolean(caps.available),
      mode: bridge.bridge_status === 'healthy' ? 'bridge' : (caps.available ? 'ephemeral' : 'unavailable'),
      note: caps.last_probe_error || (caps.available ? 'native tools available' : 'native unavailable'),
      capabilities: caps,
      bridge,
    };
    this.store?.setNativeCapabilities?.({ ...caps, bridge_status: bridge.bridge_status, bridge });
    this.store?.emitAdapterHealth?.('native', out);
    return out;
  }

  async ensureBridge(team = {}) {
    const autostart = (team?.policy?.native_bridge_policy || 'auto') !== 'off' && (process.env.LEAD_SIDECAR_NATIVE_BRIDGE_AUTOSTART || '1') !== '0';
  ...
  async getStatus(team = {}) {
    ...
    const lastValidation = readJSON(this.paths.nativeBridgeValidationFile) || null;
    return {
  ...
  async validateBridge({ team = {}, team_name = null, directory = process.cwd(), timeoutMs = null, simulate = null } = {}) {
    const resolvedTeamName = team_name || team?.team_name || null;
    const timeout = Number(timeoutMs || team?.policy?.native_action_timeout_ms || 10000);
  ...
  _shouldTryBridge(actionKey, team = {}, health = null) {
    const mode = team?.policy?.native_bridge_mode || process.env.LEAD_SIDECAR_NATIVE_BRIDGE_MODE || 'hybrid';
    if (mode === 'ephemeral_only') return false;
    if (['send-message', 'task', 'team-status', 'team-create'].includes(actionKey) === false) return false;
    if (mode === 'bridge_only') return true;
    const bridge = health?.bridge || this.bridge.getHealth();
    return bridge.bridge_status === 'healthy';
  }

  async execute(action, payload = {}, { team = {}, force_path_mode = null } = {}) {
    ...
    const timeoutMs = Number(team?.policy?.native_action_timeout_ms || payload.timeout_ms || 15000);

    let triedBridge = false;
    if (force_path_mode === 'bridge' || (!force_path_mode && this._shouldTryBridge(mapped, team, health))) {
      triedBridge = true;
      try {
        if (health.bridge?.bridge_status !== 'healthy') {
          await this.ensureBridge(team);
        }
        const res = await this.bridge.execute(mapped, normalized, { timeoutMs });
        return {
          ...
          bridge_session_id: res.bridge_session_id || this.bridge.getHealth().session_id || null,
        };
      } catch (err) {
        this.store?.raiseAlert?.({ level: 'warn', code: 'bridge_timeout', message: `Native bridge failed for ${mapped}: ${err.message}` });
        if (force_path_mode === 'bridge' || (team?.policy?.native_fallback_policy || 'coordinator') === 'error') {
          return {
            ok: false,
            action: mapped,
            native_tool: null,
            error: { code: String(err.message || 'bridge_timeout'), message: 'Bridge execution failed' },
            path_mode: 'bridge',
    ...
    const runnerRes = await this.runner.run(mapped, normalized, { timeoutMs, model: payload.model || 'sonnet' });
    return {
      ...runnerRes,
      action: mapped,
