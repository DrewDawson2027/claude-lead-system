...
function findBridgeSession(paths, workerName = "sidecar-native-bridge") {
  try {
    const files = readdirSync(paths.terminalsDir).filter(
...
export class BridgeController {
  ...
  _queueCounts() {
    try {
      return {
        request_queue: readdirSync(this.paths.nativeBridgeRequestDir).filter(
          (f) => f.endsWith(".json"),
        ).length,
        response_queue: readdirSync(this.paths.nativeBridgeResponseDir).filter(
          (f) => f.endsWith(".json"),
        ).length,
      };
    } catch {
  ...
  async ensureBridge({ autostart = false, directory = process.cwd() } = {}) {
    ...
    if (session?.session) {
      this._statusPatch({
        ...
        worker_name: session.worker_name || "sidecar-native-bridge",
        pid: session.pid || process.pid,
        note: "bridge session discovered",
      });
      this.heartbeat(session.session, {
        capabilities: ["TeamCreate", "TeamStatus", "SendMessage", "Task"],
      });
      return {
        ok: true,
        status: "healthy",
    ...
    const res = await this.coordinator
      .execute("spawn-worker-raw", {
        directory,
        prompt,
        mode: "interactive",
        runtime: "claude",
    ...
    if (task_id || session2?.session) {
      this._statusPatch({
        ...
        session_id: session2?.session || null,
        pid: session2?.pid || process.pid,
        note: "bridge spawned",
      });
      return {
        ok: true,
  ...
  async execute(
    action,
    payload = {},
    { timeoutMs = 15000, allowDegraded = false } = {},
  ) {
  ...
  _simulateBridgeWorkerResponse({ timeoutMs = 5000, once = true } = {}) {
    ...
    const timer = setInterval(() => {
      try {
        const files = readdirSync(this.paths.nativeBridgeRequestDir)
        ...
        for (const file of files) {
          ...
          const response = {
            ...
            action: req.action,
            native_tool:
              req.action === "team-status"
                ? "TeamStatus"
                : req.action === "team-create"
                  ? "TeamCreate"
          ...
          writeJSON(
            join(this.paths.nativeBridgeResponseDir, `${req.request_id}.json`),
            response,
          );
  ...
  async validate({
    ...
    const report = {
      ...
      team_name,
      simulate: Boolean(
        simulate || process.env.LEAD_SIDECAR_BRIDGE_VALIDATE_SIMULATE === "1",
      ),
      steps: [],
      diagnostics: {
        pre_health: pre,
    ...
    const step = (name, patch = {}) =>
      report.steps.push({ ts: new Date().toISOString(), name, ...patch });
    ...
    try {
      ...
      const bridgeRes = await this.execute(
        "team-status",
        {
          team_name,
      ...
      step("bridge_execute.done", {
        ok: bridgeRes.ok !== false,
        path_mode: bridgeRes.path_mode || "bridge",
      });
      ...
      try {
        writeJSON(this.paths.nativeBridgeValidationFile, report);
      } catch {}
      try {
        appendJSONL(this.paths.nativeBridgeValidationLogFile, report);
      } catch {}
