...
export function ensureBridgeDirs(paths) {
  ensureDirs([paths.nativeRuntimeDir, paths.nativeBridgeRequestDir, paths.nativeBridgeResponseDir]);
}
...
export async function waitForBridgeResponse(paths, requestId, timeoutMs = 15000, pollMs = 200) {
...
export function findStuckBridgeRequests(paths, stuckMs = 30_000) {
  ...
  return listDir(paths.nativeBridgeRequestDir)
    ...
    .map((f) => {
      ...
      return { request_id: req.request_id, action: req.action || null, team_name: req.team_name || null, age_ms, timeout_ms: req.timeout_ms || null };
    })
    .filter(Boolean)
    .sort((a, b) => b.age_ms - a.age_ms);
}

export function sweepBridgeQueues(paths, { requestMaxAgeMs = 30 * 60_000, responseMaxAgeMs = 30 * 60_000 } = {}) {
  const now = Date.now();
  const removed = { requests: 0, responses: 0 };
  for (const f of listDir(paths.nativeBridgeRequestDir).filter((x) => x.endsWith('.json'))) {
  ...
  for (const f of listDir(paths.nativeBridgeResponseDir).filter((x) => x.endsWith('.json'))) {
